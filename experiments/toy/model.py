...
class LightningDiT(nn.Module):
    ...
    def __init__(
        ...
        self.use_mlp_y_embedder = use_mlp_y_embedder
        if self.use_mlp_y_embedder:
            self.y_embedder = MLPEmbedder(cond_dim, hidden_size, hidden_size)
        else:
            self.y_embedder = nn.Linear(cond_dim, hidden_size, bias=True)
        
        num_patches = self.x_embedder.num_patches
        
        # Will use fixed sin-cos embedding:
        self.pos_embedder = nn.Parameter(torch.zeros(1, num_patches, hidden_size), requires_grad=False)
    ...
    def initialize_weights(self):
        ...
        # Initialize (and freeze) pos_embedder by sin-cos embedding:
        pos_embedder = get_2d_sincos_pos_embed(self.pos_embedder.shape[-1], int(self.x_embedder.num_patches ** 0.5))
        self.pos_embedder.data.copy_(torch.from_numpy(pos_embedder).float().unsqueeze(0))
        ...
        if self.use_mlp_y_embedder:
            ...
            for module in self.y_embedder.modules():
    ...
    def forward(self, x, t=None, y=None):
        ...
        x = self.x_embedder(x) + self.pos_embedder  # (N, T, D), where T = H * W / patch_size ** 2
...
def get_2d_sincos_pos_embed(embed_dim, grid_size, cls_token=False, extra_tokens=0):
    ...
    return:
    pos_embedder: [grid_size*grid_size, embed_dim] or [1+grid_size*grid_size, embed_dim] (w/ or w/o cls_token)
    """
    ...
    pos_embedder = get_2d_sincos_pos_embed_from_grid(embed_dim, grid)
    if cls_token and extra_tokens > 0:
        pos_embedder = np.concatenate([np.zeros([extra_tokens, embed_dim]), pos_embedder], axis=0)
    return pos_embedder
...
def LightningDiT_S_2(**kwargs):
    return LightningDiT(depth=8, hidden_size=512, patch_size=2, num_heads=12, **kwargs)

def LightningDiT_S_4(**kwargs):
    return LightningDiT(depth=8, hidden_size=512, patch_size=4, num_heads=8, **kwargs)

def LightningDiT_XS_4(**kwargs):
    return LightningDiT(depth=6, hidden_size=512, patch_size=4, num_heads=8, **kwargs)
...
LightningDiT_models = {
    ...
    'LightningDiT-XS/4': LightningDiT_XS_4,
