...
def arg_parser():
    ...
    parser.add_argument('--ti_dir', type=str, default=str(experiment_dir / 'results' / 'ti'))
...
if __name__ == "__main__":
    ...
    # setting 
    # ------------------------------------------------------------
    save_path = f'{args.output_dir}/{args.layer}-slider_ti-NFE{args.NFE}'
    if args.normalize:
        save_path += '-norm'
    ...
    _, prompt_embs = get_synth_latent_text_embed_ti(
        sd_version,
        {
            ...
            'new2.bin': f'{args.ti_dir}/{task_idx}',
        },
        image_path=None,
        captions=[
            task['prompt'],
