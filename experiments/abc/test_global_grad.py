...
if __name__ == "__main__":
    ...
    # setting 
    # ------------------------------------------------------------
    save_path = f'{args.output_dir}/{args.layer}-slider-NFE{args.NFE}'
    if args.normalize:
        save_path += '-norm'
    ...
    _, p_pos, p_neg, p_base = get_synth_latent_text_embed(
        sd_version,
        f'{data_path}/{task["model_path"]}',
        f'{data_path}/{task["synth_image_path"]}',
        task['prompt'],                         # task['train_prompt'], # TODO: kind of hacky
        task['prompt'].replace('<new1> ', ''),  # task['train_prompt'].replace('<new1> ', ''), # TODO: kind of hacky
        batch_size=1,
        device='cuda',
        dtype=dtype,
    )
    ...
    for i in tqdm(range(args.epochs), desc='concept grad slider...'):
        ...
        with torch.no_grad():
            ...
            xT = torch.randn(args.batch_size, 4, 64, 64, device=device, dtype=dtype)
